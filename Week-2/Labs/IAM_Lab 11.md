...
| Tools Used | nslookup, ping, ipconfig /all |
...
The Windows 11 client successfully contacted the internal DNS server and resolved mydomain.com to the Domain Controller's IP address.

**Result:**

**DNS resolution was successfully verified.**

**Screenshot 2:** Successful nslookup mydomain.com

**Step 3: Intentionally Breaking DNS**

To create a controlled troubleshooting scenario, the client's DNS configuration was changed to an incorrect DNS server.

**Procedure:**

On **Win11-Client**:

1. Opened **Network and Internet Settings**.

2. Opened **Advanced network settings**.

3. Selected **More network adapter options**.

4. Right-clicked the Ethernet adapter.

5. Selected **Properties**.

6. Opened **Internet Protocol Version 4 (TCP/IPv4)**.

7. Selected **Properties**.

8. Changed the **Preferred DNS server** from:
...
The client was now configured to use Google's public DNS server instead of the internal Active Directory DNS server.

**Result:**

DNS configuration was intentionally modified to create the troubleshooting scenario.

**Part 2: DNS Failure Testing**

**Step 4: Testing DNS Using nslookup**

After changing the DNS server, DNS resolution was tested again.
...
The client attempted to send the DNS request to 8.8.8.8, but the internal domain mydomain.com could not be resolved.
...
**Step 8: Restoring the Correct DNS Server**

**Procedure:**

1. Opened **IPv4 Properties** on Win11-Client.

2. Changed the Preferred DNS server from:
...
The Windows 11 client was once again configured to use the internal Active Directory DNS server.

**Screenshot 8:** Corrected DNS configuration showing 172.16.0.1
...
|----|----|----|
| nslookup mydomain.com failed | Client was using 8.8.8.8 | Changed DNS to 172.16.0.1 |
| ping mydomain.com failed | Domain name could not be resolved | Restored internal DNS |
| ping 172.16.0.1 worked | Network connectivity was functioning | Confirmed issue was DNS, not network |
| ipconfig /all showed wrong DNS | Incorrect IPv4 DNS configuration | Changed DNS server back to 172.16.0.1 |

**Key Takeaways**

| **Test** | **What It Demonstrated** |
|-----------------------|------------------------------------------|
| nslookup mydomain.com | DNS name resolution |
| ping mydomain.com | DNS + network connectivity |
| ping 172.16.0.1 | Direct IP connectivity |
| ipconfig /all | DNS and network configuration |
| 8.8.8.8 | Incorrect DNS server for internal domain |
| 172.16.0.1 | Internal Active Directory DNS server |
...
![Screenshot 1](IAM_Lab_11_1.png)

![Screenshot 2](IAM_Lab_11_2.png)

![Screenshot 3](IAM_Lab_11_3.png)

![Screenshot 4](IAM_Lab_11_4.png)
